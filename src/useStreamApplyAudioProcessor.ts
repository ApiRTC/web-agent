...
export default function useStreamApplyAudioProcessor(
    ...
    const outStreamRef = useRef<Stream>();
    ...
    useEffect(() => {
        // setOutStream(stream)
        // if (outStreamRef.current && outStreamRef.current !== stream) {
        //     outStreamRef.current.release()
        // }
    ...
    useEffect(() => {
        ...
        if (stream && processorType !== applied) {
            ...
            stream.applyAudioProcessor(processorType).then(l_stream => {
                setOutStream(processorType !== 'none' ? l_stream : undefined)
                outStreamRef.current = l_stream;
